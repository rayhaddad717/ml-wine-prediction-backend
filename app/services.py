...
def predict(sample):
    # Load the data and preprocess it in the same way as the training data
    df = pd.read_csv('resources/wine-quality.csv')
    df = df.drop('total sulfur dioxide', axis=1)
    df['best quality'] = [1 if x > 5 else 0 for x in df.quality]
    df.replace({'white': 1, 'red': 0}, inplace=True)
    ...
    new_data_norm = norm.transform(sample)
...
#     keys_obj = WineSample(data_dict)

#     df = keys_obj.get_dataframe()
