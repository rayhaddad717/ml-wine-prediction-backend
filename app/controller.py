...
class WineController:
    ...
    def bulk_wine_predict(self,file):
        ...
        for i, row in df.iterrows():
            ...
            if(validated_schema.validated == False):
                return {'success':False,'error': validated_schema.errors,'row':i + 1}
            predictions.append(int(model.predict(current_df)))      
    ...
    def predict_from_image(self,text):
        try:
            ...
            keywords = ['fixed acidity', 'volatile acidity', 'citric acid', 'residual sugar', 'chlorides',
