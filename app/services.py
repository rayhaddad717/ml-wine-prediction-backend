...
class MLModel:
    ...
    def createAndTrainModel(self):
        ...
        linear_xtrain, linear_xtest, linear_ytrain, linear_ytest = train_test_split(
            features, df["quality"], test_size=0.2, random_state=40
        )
        linear_norm = MinMaxScaler()
        linear_xtrain = linear_norm.fit_transform(linear_xtrain)
        linear_xtest = linear_norm.transform(linear_xtest)
    ...
    def predict(self, sample, linear=False):
        # Preprocess the new data and normalize it using the same MinMaxScaler as the training data
        sample = sample[self.columns]
        new_data_norm = (
            self.norm.transform(sample)
            if not linear
            else self.linear_norm.transform(sample)
        )

        # Load the trained model and use it to predict on the new data
        prediction = (
            self.model.predict(new_data_norm)
            if not linear
            else self.linear_model.predict(new_data_norm)
        )

        # this will print the predicted class label for the new data (0 or 1)
